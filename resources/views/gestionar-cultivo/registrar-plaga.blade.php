    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-200">
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto p-6">
            ...
            <nav class="text-sm mb-4">
                ...
                <span class="text-gray-700">Registrar Plaga</span>
            </nav>
            
            <h1 class="text-2xl lg:text-3xl font-semibold mb-6">Registrar Plaga</h1>


            <!-- Opciones de Gestionar Cultivos en una línea con imágenes al comienzo -->
            <div class="flex mt-4 space-x-4">
                <a href="{{ route('nuevaPlaga')}}" class="flex flex-col items-center py-4 px-4 text-sm lg:text-base bg-yellow-500 hover:bg-yellow-400 text-white rounded">
            ...
            <!-- Tabla de Cultivos -->
            <div class="mt-8">
                <h2 class="text-xl font-semibold mb-4">Plagas Registradas</h2>
                ...
                <table class="min-w-full bg-white border border-gray-300">
                    <thead>
                        <tr>
                            ...
                            <th class="py-2 px-4 border-b text-left">Nombre Plaga</th>
                    ...
                    <tbody>
                        @foreach($plagas as $plaga)
                            <tr>
                                <td class="py-2 px-4 border-b">{{ $plaga->IDPlaga }}</td>
                                <td class="py-2 px-4 border-b">{{ $plaga->NombrePlaga }}</td>
                                <td class="py-2 px-4 border-b">{{ $plaga->Descripcion }}</td>
                                <td class="py-2 px-4 border-b flex items-center space-x-2">
                                <form action="{{ route('eliminarPlaga', ['id' => $plaga->IDPlaga]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-6 h-6 cursor-pointer ml-2" onclick="return confirm('¿Estás seguro de eliminar esta plaga?')">
