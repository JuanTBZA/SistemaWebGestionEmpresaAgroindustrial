...
<body class="bg-gray-100 font-sans">
    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-200">
        ...
        <div class="flex-1 flex flex-col overflow-x-hidden overflow-y-auto p-6">
            ...
            <h1 class="text-2xl lg:text-3xl font-semibold mb-6">Registrar parcela</h1>
      

            <!-- Opciones de Gestionar Cultivos en una línea con imágenes al comienzo -->
            <div class="flex mt-4 space-x-4">
                <a href="{{ route('nuevaParcela')}}" class="flex flex-col items-center py-4 px-4 text-sm lg:text-base bg-green-500 hover:bg-green-400 text-white rounded">
            ...
            <!-- Tabla de parcelas -->
            <div class="mt-8">
                    <h2 class="text-xl font-semibold mb-4">Parcelas Registradas</h2>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="py-2 px-4 border-b text-left">Nombre Parcela</th>
                            <th class="py-2 px-4 border-b text-left">Ubicación</th>
                            <th class="py-2 px-4 border-b text-left">Tamaño (m²)</th>
                    ...
                    <tbody>
                        @foreach($parcelas as $parcela)
                            <tr>
                                <td class="py-2 px-4 border-b">{{ $parcela->IDParcela }}</td>
                                <td class="py-2 px-4 border-b">{{ $parcela->NombreParcela }}</td>
                                <td class="py-2 px-4 border-b">{{ $parcela->Ubicacion }}</td>
                                <td class="py-2 px-4 border-b">{{ $parcela->Tamano }}</td>
                                <td class="py-2 px-4 border-b flex items-center space-x-2">
                                    <form action="{{ route('eliminarParcela', ['id' => $parcela->IDParcela]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-6 h-6 cursor-pointer ml-2" onclick="return confirm('¿Estás seguro de eliminar esta parcela?')">
