...
<body class="bg-gray-100 font-sans">
    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-200">
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto p-6">
            ...
            <h1 class="text-2xl lg:text-3xl font-semibold mb-6">Registrar cultivo</h1>

            <!-- Opciones de Gestionar Cultivos en una línea con imágenes al comienzo -->
            <div class="flex mt-4 space-x-4">
                <a href="{{ route('registrarParcela')}}" class="flex flex-col items-center py-4 px-4 text-sm lg:text-base bg-green-500 hover:bg-green-400 text-white rounded">
                    <img src="{{asset('/img/gestionar-cultivo/parcela.png')}}" alt="Imagen 1" class="w-10 h-10 mb-2" />
                    Registrar Parcela
                </a>
                
                <a href="{{ route('buscarParcela')}}" class="flex flex-col items-center py-4 px-4 text-sm lg:text-base bg-green-500 hover:bg-green-400 text-white rounded">
                ...
                <!-- Tabla de Cultivos -->
                <div class="mt-8">
                    <h2 class="text-xl font-semibold mb-4">Cultivos Registrados</h2>
                    ...
                    <table class="min-w-full bg-white border border-gray-300">
                        <thead>
                            <tr>
                                ...
                                <th class="py-2 px-4 border-b text-left">Nombre Cultivo</th>
                                <th class="py-2 px-4 border-b text-left">Fecha Plantación</th>
                                <th class="py-2 px-4 border-b text-left">ID Parcela</th>
                        ...
                        <tbody>
                            @foreach($cultivos as $cultivo)
                                <tr>
                                    <td class="py-2 px-4 border-b">{{ $cultivo->IDCultivo }}</td>
                                    <td class="py-2 px-4 border-b">{{ $cultivo->NombreCultivo }}</td>
                                    <td class="py-2 px-4 border-b">{{ $cultivo->FechaPlantacion }}</td>
                                    <td class="py-2 px-4 border-b">{{ $cultivo->IDParcela }}</td>
                                    <td class="py-2 px-4 border-b flex items-center space-x-2">
                                    <form action="{{ route('eliminarCultivo', ['id' => $cultivo->IDCultivo]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-6 h-6 cursor-pointer ml-2" onclick="return confirm('¿Estás seguro de eliminar este cultivo?')">
