...
<body class="bg-gray-100 font-sans">
    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-200">
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto p-6">
            ...
            <h1 class="text-2xl lg:text-3xl font-semibold mb-6">Programar Labores</h1>
        

            <!-- Opciones de Gestionar Cultivos en una línea con imágenes al comienzo -->
            <div class="flex mt-4 space-x-4">

                <a href="{{ route('buscarCultivo', ['redirect' => 'nuevaTarea']) }}" class="flex flex-col items-center py-4 px-4 text-sm lg:text-base bg-yellow-500 hover:bg-yellow-400 text-white rounded">
            ...
            @if(session('success'))
                    ...
                    @endif
                         ...
                         <!-- Tabla de Labores -->
                         <div class="mt-8">
                <h2 class="text-xl font-semibold mb-4">Tareas Registradas</h2>
                ...
                <table class="min-w-full bg-white border border-gray-300">
                    <thead>
                        <tr>
                            <th class="py-2 px-4 border-b text-left">ID Tarea</th>
                    ...
                    <tbody>
                        @foreach($tareasGestionCultivo as $tarea)
                            <tr>
                                <td class="py-2 px-4 border-b">{{ $tarea->IDTarea }}</td>
                                <td class="py-2 px-4 border-b">{{ $tarea->IDCultivo }}</td>
                                <td class="py-2 px-4 border-b">{{ $tarea->FechaInicio }}</td>
                                <td class="py-2 px-4 border-b">{{ $tarea->FechaFin }}</td>
                                <td class="py-2 px-4 border-b">{{ $tarea->Descripcion }}</td>
                                <td class="py-2 px-4 border-b">{{ $tarea->Completada ? 'Completada' : 'Pendiente' }}</td>
                                <td class="py-2 px-4 border-b flex items-center space-x-2">
                                <form action="{{ route('eliminarTarea', ['id' => $tarea->IDTarea]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-6 h-6 cursor-pointer ml-2" onclick="return confirm('¿Estás seguro de eliminar esta tarea?')">
