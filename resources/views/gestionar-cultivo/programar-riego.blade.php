...
<body class="bg-gray-100 font-sans">
    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-200">
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto p-6">
            ...
            <h1 class="text-2xl lg:text-3xl font-semibold mb-6">Programar Riegos</h1>
            ...
            <div class="flex mt-4 space-x-4">
                ...
                <a href="{{ route('buscarCultivo', ['redirect' => 'nuevoRiego']) }}" class="flex flex-col items-center py-4 px-4 text-sm lg:text-base bg-yellow-500 hover:bg-yellow-400 text-white rounded">
                    <img src="{{ asset('/img/mantenedores/agregar.png') }}" alt="Imagen 1" class="w-10 h-10 mb-2" />
                    Nuevo Riego
                </a>
                         ...
                         <!-- Tabla de Labores -->
                         <div class="mt-8">
                <h2 class="text-xl font-semibold mb-4">Riegos Registrados</h2>
                ...
                <table class="min-w-full bg-white border border-gray-300">
                    <thead>
                        <tr>
                            <th class="py-2 px-4 border-b text-left">ID Riego</th>
                            <th class="py-2 px-4 border-b text-left">ID Cultivo</th>
                            <th class="py-2 px-4 border-b text-left">Fecha de Riego</th>
                            <th class="py-2 px-4 border-b text-left">Duración (minutos)</th>
                            <th class="py-2 px-4 border-b text-left">Galones Utilizados</th>
                    ...
                    <tbody>
                        @foreach($riegos as $riego)
                            <tr>
                                <td class="py-2 px-4 border-b">{{ $riego->IDRiego }}</td>
                                <td class="py-2 px-4 border-b">{{ $riego->IDCultivo }}</td>
                                <td class="py-2 px-4 border-b">{{ $riego->FechaRiego }}</td>
                                <td class="py-2 px-4 border-b">{{ $riego->Duracion }}</td>
                                <td class="py-2 px-4 border-b">{{ $riego->CantidadAguaUtilizada }}</td>
                                <td class="py-2 px-4 border-b flex items-center space-x-2">
                                <form action="{{ route('eliminarRiego', ['id' => $riego->IDRiego]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-6 h-6 cursor-pointer ml-2" onclick="return confirm('¿Estás seguro de eliminar este riego?')">
