...
<body class="bg-gray-100 font-sans">
    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-200">
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto p-6">
            ...
            <h1 class="text-2xl lg:text-3xl font-semibold mb-6">Registrar Recursos</h1>


            <!-- Opciones de Gestionar Cultivos en una línea con imágenes al comienzo -->
            <div class="flex mt-4 space-x-4">
                <a href="{{ route('nuevoRecurso')}}" class="flex flex-col items-center py-4 px-4 text-sm lg:text-base bg-yellow-500 hover:bg-yellow-400 text-white rounded">
                        ...
                        @foreach($recursos as $recurso)
                            <tr>
                                ...
                                <td class="py-2 px-4 border-b flex items-center space-x-2">
                                    ...
                                    <form action="{{ route('eliminarRecurso', ['id' => $recurso->IDRecurso]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="w-6 h-6 cursor-pointer ml-2" onclick="return confirm('¿Estás seguro de eliminar este recurso?')">
